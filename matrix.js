...
function performOperation(operation) {
    ...
    if (result) {
        ...
        console.error('Operation could not be performed');
    }
}
...
function addMatrices(matrix1, matrix2){ 
        if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
            console.error('Matrices are not the same size, cannot perform addition');
            return;
        }
    ...
    const subtractMatrices = function (matrix1, matrix2) {
        if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
            console.error('Matrices are not the same size, cannot perform subtraction');
            return;
        }
    ...
    const multiplyMatrices = (matrix1, matrix2) => {
        if (matrix1[0].length !== matrix2.length) {
            console.error('The number of columns in the first matrix is not equal to the number of rows in the second matrix, cannot perform multiplication');
            return;
        }
